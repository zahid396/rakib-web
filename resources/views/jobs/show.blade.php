    ...
    @if($job->isOpen())
        @if(session('success'))
            <div class="apply-box" style="text-align:center;">
                ...
                <p class="apply-hint" style="margin:0;">You have already applied successfully for this position. We'll reach out to you via email. If you need to change anything, please contact us directly.</p>
            </div>
        @else
            <div class="apply-box">
                <h3>Apply for this Position</h3>
                <p class="apply-hint">Fill in your details below. We'll reach out to you via email.</p>
                <form id="applyForm" method="POST" action="{{ route('jobs.apply', $job) }}">
                    @csrf
                    ...
                    <button type="submit" class="apply-submit" id="applySubmitBtn">Submit Application</button>
                </form>
            </div>
        @endif
    @else
