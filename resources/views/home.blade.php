...
@section('head')
<style>
/* ── Page background: Square Grid Pattern ── */
body {
...
.about-section p { color: var(--gray-600); line-height: 1.8; font-size: 1rem; max-width: 600px; margin: 0 auto; white-space: pre-wrap; overflow-wrap: break-word; word-break: break-word; text-align: left; }
...
#productModal .modal-desc { color: var(--gray-600); line-height: 1.75; font-size: .95rem; margin-bottom: 1.25rem; white-space: pre-wrap; overflow-wrap: break-word; word-break: break-word; }
...
(function(){
    ...
    if(!track) return;
    var slides=track.children.length, cur=0, auto, videoPlaying=false;
    ...
    function startAuto(){ if(videoPlaying) return; clearInterval(auto); auto=setInterval(function(){go(cur+1);},5000); }
    function resetAuto(){ clearInterval(auto); startAuto(); }
    dots.forEach(function(d){d.addEventListener('click',function(){stopVideos();go(+d.dataset.slide);});});
    if(prev) prev.addEventListener('click',function(){stopVideos();go(cur-1);});
    if(next) next.addEventListener('click',function(){stopVideos();go(cur+1);});
    var hx=0;
    track.addEventListener('touchstart',function(e){hx=e.touches[0].clientX;},{passive:true});
    track.addEventListener('touchend',function(e){var dx=e.changedTouches[0].clientX-hx;if(Math.abs(dx)>50){stopVideos();dx>0?go(cur-1):go(cur+1);}});
    startAuto();

    /* Banner Video: load the player only when the visitor clicks play.
       While a banner video is playing, the carousel stops auto-changing so
       the visitor is not interrupted. Manual navigation stops the video. */
    function attachPlay(frame){
    ...
    function stopVideos(){
        document.querySelectorAll('.video-frame').forEach(function(frame){
            ...
            var vid = frame.getAttribute('data-video');
            frame.innerHTML = '<img class="video-poster" src="https://i.ytimg.com/vi/'+vid+'/hqdefault.jpg" alt="Banner video"><button type="button" class="video-playbtn" aria-label="Play video">▶</button>';
            attachPlay(frame);
        });
        videoPlaying=false;
        resetAuto();
    }
    document.querySelectorAll('.video-frame').forEach(attachPlay);
