...
.order-id-box span { font-size: .78rem; color: var(--gray-500); display: block; text-transform: uppercase; letter-spacing: .5px; margin-bottom: .4rem; }
.order-id-copy { display: flex; align-items: center; justify-content: center; gap: .6rem; }
.order-id-copy strong { font-size: 1.35rem; font-weight: 800; color: var(--primary); letter-spacing: 1px; }
.copy-id-btn { display: inline-flex; align-items: center; gap: .35rem; padding: .4rem .85rem; border-radius: 50px; background: linear-gradient(135deg, #f0b90b, #e2a008); color: #0b0f1c; border: none; cursor: pointer; font-family: var(--font-display); font-size: .75rem; font-weight: 700; white-space: nowrap; transition: var(--transition); }
.copy-id-btn:hover { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(240,185,11,.35); }
.copy-id-btn.copied { background: var(--accent); color: #fff; }
...
<div class="success-page">
    <div class="container">
        <div class="success-card">
            ...
            @php $orderIdValue = $orderId ?? ($order->order_id ?? ($order->id ?? '')); @endphp
            <div class="order-id-box">
                <span>Your Order ID</span>
                <div class="order-id-copy">
                    <strong>#{{ $orderIdValue }}</strong>
                    <button type="button" class="copy-id-btn" id="copyOrderId" data-order-id="{{ $orderIdValue }}" title="Copy Order ID" aria-label="Copy Order ID">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                        Copy
                    </button>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var copyBtn = document.getElementById('copyOrderId');
        if (!copyBtn) return;

        copyBtn.addEventListener('click', function () {
            var orderId = copyBtn.getAttribute('data-order-id') || '';
            ...
            if (navigator.clipboard && window.isSecureContext) {
                navigator.clipboard.writeText(orderId).then(showCopied).catch(fallback);
            } else {
                fallback();
            }
        });
        ...
        function showCopied() {
            ...
            copyBtn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Copied!';
            setTimeout(function () {
